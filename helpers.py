...
class CameraHandler:
    ...
    def getFPS(self):
        ...
        return f"Cur: {self.fps}, Min: {min(pastFpses)}, Max: {max(pastFpses)}, Avg {round(sum(pastFpses)/1000, 2)})"


    def startRecording(self):
        count = str(len(os.listdir("./Videos")))
        self.recorder = cv2.VideoWriter(
            "ThermalCamVideo " + count, CameraHandler.fourcc, 50, (256, 192)
            )
        self.record = True
...
class UARTController:
    ...
    commands = {
        ...
        'Frame Rate': {
            '50': rb'\x55\x43\x49\x12\x00\x10\x10\x44\x00\x32\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x80\xE4'
        },
        'Data Source': {
            'Get': rb'\x55\x43\x49\x12\x00\x10\x10\x85\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x8E\xCA',
            'YUV': rb'\x55\x43\x49\x12\x00\x10\x10\x45\x00\x05\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x5A\xCE'
        }
    }
    responsesGeneral = {}
    responses = {}
